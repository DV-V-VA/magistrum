#!/usr/bin/env python3
import asyncio, aiohttp, json, os, re, logging
...
PUBTATOR    = PUBTATOR = "https://www.ncbi.nlm.nih.gov/research/pubtator3-api/publications/export/biocjson?pmids={pmids}&full=true"
...
FUNC_WORDS = r"\b(kcat|k\s*cat|k\s*/?\s*m|kcat\s*/\s*k\s*m|ic50|ec50|kd|affinity|binding|activity|turnover|stability|tm|melting|thermal\s*shift|fluorescence|reporter|growth|fitness|complementation|enzymatic)\b"
HGVS_PROT  = r"p\.\(?[A-Z][a-z]{2}\d+[A-Z][a-z]{2}\)?"
ONE_LETTER = r"\b[A-Z]\d+[A-Z]\b"
SCAN_WORDS = r"(alanine scanning|site-directed mutagenesis|saturation mutagenesis|phospho[- ]mutant|deletion mutant|truncation)"
...
class RL:
    def __init__(self, rps: float): self.rps, self._t = max(0.1, rps), 0.0
    async def tick(self):
        ...
        try:
            async with session.get(url, **kw) as r:
                ...
                if r.status in (429,500,502,503,504):
                    await asyncio.sleep(min(8, 0.5*(2**attempt))); continue
                if r.status == 204:
                    return None
                text = await r.text()
                if not text.strip():
                ...
                try:
                    ...
                    raise RuntimeError(f"Non-JSON from {url} (status {r.status}, CT={r.headers.get('Content-Type')}): {text[:200]!r}") from e
        ...
        for doc in docs:
            ...
            for p in anns:
                for ann in p.get("annotations", []):
                    ...
                    if typ in ("mutation","variant","genetic_variant","genetic variant","variantmention"):
                        flag = True; break
                if flag: break
            out[pid] = flag
    return out
...
def quick_score(a: Article, syn_re) -> (int, list):
    ...
    reasons = []; score = 0
    if syn_re.search(txt): score += 25; reasons.append("protein_synonym")
    if RE_HGVS.search(txt) or RE_ONE.search(txt) or RE_SCAN.search(txt):
        score += 25; reasons.append("mutation_pattern")
    if RE_FUNC.search(txt): score += 25; reasons.append("functional_metric")
    if any(pt.lower() in ("review", "editorial", "comment") for pt in (a.pub_types or [])):
        score -= 30; reasons.append("not_primary_research")
    if a.year and a.year >= 2015: score += 5
    return score, reasons

def compile_synonyms(syns: List[str]):
    syns2 = [re.escape(x) for x in syns if x.strip()]
    return re.compile(r"\b(" + "|".join(syns2) + r")\b", re.IGNORECASE) if syns2 else re.compile("$a")

def save_bytes(path: str, data: str):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w", encoding="utf-8") as f: f.write(data)

async def triage(in_jsonl: str, out_prefix: str, protein_syns: List[str],
                 upw_email: Optional[str] = None, use_semantic: bool = False,
                 sem_model: str = "pritamdeka/S-Biomed-Roberta-snli-multinli-stsb"):
    syn_re = compile_synonyms(protein_syns)
    st_model = None; q_emb = None
    if use_semantic:
    ...
    epmc_rl = RL(RPS_EPMC); ptc_rl = RL(RPS_PTC); upw_rl = RL(RPS_UPW)
    ...
    async with aiohttp.ClientSession(timeout=TIMEOUT, trust_env=True) as session:
        ...
        logger.info("PubTator tmVar flags: %d/%d", sum(1 for v in tmvar_map.values() if v), len(pmid_list))
        epmc_ids = [("PMC:"+a.pmcid.replace("PMC","")) if a.pmcid else ("MED:"+a.pmid) for a in arts if a.pmid]
        ann_map = await epmc_annotations_any(session, epmc_rl, epmc_ids)
        logger.info("EuropePMC annotations present for: %d/%d", sum(1 for k,v in ann_map.items() if v), len(epmc_ids))
        ...
        for a in arts:
            ...
            has_ann = ann_map.get("PMID:"+a.pmid, False) or (a.pmcid and ann_map.get("PMC:"+a.pmcid.replace("PMC",""), False))
            if has_tmvar: base_score += 15; reasons.append("tmVar_mutation")
            if has_ann:   base_score += 10; reasons.append("EPMC_annotations")
            ...
            if use_semantic and st_model:
                ...
                base_score += delta; reasons.append(f"sem_sim_{delta}")

            is_rev = any(pt.lower()=="review" for pt in (a.pub_types or []))
            oa = False; oa_route = None; fulltext_path = None; pmcid = a.pmcid

            if (not is_rev) and (base_score > SCORE_MIN_FOR_FULLTEXT):
                if not pmcid and a.doi:
                    logger.debug("[%s] score=%d > %d, resolving PMCID via EPMC core by DOI", a.pmid, base_score, SCORE_MIN_FOR_FULLTEXT)
                ...
                if pmcid:
                    ...
                    if xml and len(xml) > 1000:
                        oa = True; oa_route = "EPMC_XML"
                        fulltext_path = os.path.join(out_prefix, "fulltext_xml", f"{pmcid}.xml")
                        os.makedirs(os.path.dirname(fulltext_path), exist_ok=True)
                        with open(fulltext_path, "w", encoding="utf-8") as f: f.write(xml)
                        reasons.append("OA_fulltext_EPMC"); fetched += 1
                        logger.info("[%s] saved EPMC XML → %s", a.pmid, fulltext_path)
                if (not oa) and a.doi and upw_email:
                    await upw_rl.tick()
                    j = await fetch_json(session, UNPAYWALL.format(doi=a.doi), params={"email": upw_email})
                    if j and j.get("is_oa"):
                        oa = True; oa_route = "UNPAYWALL_PDF"
                        reasons.append("OA_via_Unpaywall")
                        logger.info("[%s] OA via Unpaywall", a.pmid)
            else:
                logger.debug("[%s] score=%d%s — skip fulltext fetch", a.pmid, base_score, ", review" if is_rev else "")

            results.append(Triage(
                pmid=a.pmid, doi=a.doi, pmcid=pmcid or a.pmcid, oa=oa, oa_route=oa_route,
                fulltext_path=fulltext_path, score=base_score, reasons=reasons,
                has_tmvar=has_tmvar, has_epmc_ann=has_ann, is_review=is_rev
            ))
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="Triage & fulltext fetch (Europe PMC + PubTator + optional semantics)")
    ...
    import asyncio
    asyncio.run(triage(args.in_jsonl, args.out, syns, args.unpaywall_email, args.use_semantic, args.sem_model))
